...
class Context {
	...
	/**
	 * Determins if this context is serving a request for an AMP page
	 * @var bool
	 */
	...
	public function __construct($rootPath) {
		...
		if ($this->setting('amp/enabled', false)) {
			add_action('init',function() {
				add_rewrite_endpoint('amp',EP_PERMALINK);

				foreach($this->setting('amp/post-types',[]) as $postType)
	 ...
	 * @return bool
	 */
	public function isAMPContext() {
		return $this->isAMPContext;
	}
	...
	private function setupRequiredPlugins() {
		add_action( 'tgmpa_register', function(){
			...
			$otherPlugins = $this->themeSetting('plugins');
			if ($otherPlugins)
