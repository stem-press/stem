...
class CacheControl
{
    ...
    private function renderCacheMetabox($page)
    {
		...
		<select name="stem-cache-control" id="stem-cache-control">
			<?php if ($hasDefault): ?>
				<option value="default" <?php if ($cc) {
            selected($cc, 'default');
        } ?>>Default</option>';
			<?php endif; ?>
			<option value="none" <?php if ($cc) {
            selected($cc, 'none');
        } ?>>None</option>';
			<option value="no-store" <?php if ($cc) {
            selected($cc, 'no-store');
        } ?>>No Store</option>';
			<option value="no-cache" <?php if ($cc) {
            selected($cc, 'no-cache');
        } ?>>No Cache</option>';
			<option value="no-store, no-cache" <?php if ($cc) {
            selected($cc, 'no-store, no-cache');
        } ?>>No Store, No Cache</option>';
			<option value="private" <?php if ($cc) {
            selected($cc, 'private');
        } ?>>Private</option>';
			<option value="public" <?php if ($cc) {
            selected($cc, 'public');
        } ?>>Public</option>';
		</select>
    ...
    public function addResponseHeaders(Response $response)
    {
        ...
        if ($this->cacheControl) {
            ...
            } elseif ($this->cacheControl == 'private') {
                $response->headers->addCacheControlDirective('private', true);

                if (is_numeric($this->maxAge)) {
                    $response->setMaxAge($this->maxAge);
                }
            } elseif ($this->cacheControl == 'no-cache') {
                $response->headers->addCacheControlDirective('no-cache', true);
            } elseif ($this->cacheControl == 'no-store') {
                $response->headers->addCacheControlDirective('no-store', true);
            } elseif ($this->cacheControl == 'no-store, no-cache') {
                $response->headers->addCacheControlDirective('no-store', true);
                $response->headers->addCacheControlDirective('no-cache', true);
            }
        }
