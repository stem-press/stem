...
class PluginManager {
    ...
    private function set_args( $args = array() ) {
        ...
        $this->args = wp_parse_args( $args, array(
            ...
            'views_title'          => sprintf( __( 'Suggested by %s', 'stem' ), $this->package['name'] ),
            // translators: 1: name of theme
            'tab_title'            => sprintf( __( 'Suggested by %s', 'stem' ), $this->package['name'] ),
            // translators: 1: name of theme
            'extended_title'       => sprintf( __( 'Suggested Plugins by %s', 'stem' ), $this->package['name'] ),
            'menu_title'           => '', // Takes on page_title, when left blank.
            'parent_slug'          => 'plugins.php',
            'menu_slug'            => 'suggested-plugins',
            'capability'           => 'install_plugins',
            'nag_action'           => __( 'Manage suggested plugins', 'stem' ),
            'nag_dismiss'          => __( 'Dismiss this notice', 'stem' ),
            // translators: 1: name of theme
            'nag_update'           => __( 'Not all of your active, suggested plugins are compatible with %s.', 'stem' ),
            // translators: 1: name of theme, 2: number of suggested plugins
            'nag_install_single'   => __( '%1$s suggests installing %2$s plugin.', 'stem' ),
            // translators: 1: name of theme, 2: number of suggested plugins
            'nag_install_multiple' => __( '%1$s suggests installing %2$s plugins.', 'stem' ),
            'child_theme'          => false,
            'plugin_file'          => '',
        ));
    ...
    private function set_notices() {
        ...
        $notices = new PluginNotices( $args, $this, $this->plugins );
    ...
    public function add_assets() {
        ...
        wp_enqueue_script(
            'Stem-plugin-manager',
            ILAB_STEM_PUB_JS_URL."plugin-manager$suffix.js",
        ...
        wp_localize_script(
            ...
            array(
                'thirdParty'   => __( 'Only plugins from wordpress.org can be installed directly here.', 'stem' ),
                'notInstalled' => __( 'Plugin update skipped because it is not installed.', 'stem' ),
            )
        );

        wp_enqueue_style(
            'Stem-plugin-manager',
            ILAB_STEM_PUB_CSS_URL."plugin-manager$suffix.css"
        );
    ...
    public function display_page() {
        ...
        <div id="suggested-plugins" class="wrap">
            ...
            <?php if ( $plugins ) : ?>
                ...
                <form method="post" id="bulk-action-form" data-namespace="Stem">
            ...
            <?php else : ?>
                ...
                <p><?php esc_html_e( 'No suggested plugins given.', 'stem' ); ?></p>
    ...
    private function display_table_nav( $which ) {
        ...
        $actions = array(
            'install-selected'    => __( 'Install', 'stem' ),
            'activate-selected'   => __( 'Activate', 'stem' ),
            'deactivate-selected' => __( 'Deactivate', 'stem' ),
            'update-selected'     => __( 'Update', 'stem' ),
            'delete-selected'     => __( 'Delete', 'stem' ),
        );
        ...
        <div class="tablenav">
            <div class="actions bulkactions">
                ...
                <label for="bulk-action-selector-<?php echo esc_attr( $which ); ?>" class="screen-reader-text">
                    <?php esc_html_e( 'Select bulk action', 'stem' ); ?>
                </label>

                <select name="action-<?php echo $which; ?>" id="bulk-action-selector-<?php echo esc_attr( $which ); ?>">

                    <option value="-1"><?php esc_html_e( 'Bulk Actions', 'stem' ); ?></option>
                ...
                <?php
                submit_button( __( 'Apply', 'stem' ), 'action', '', false,
                    array(
                        'id' => "do-action-$which",
                    )
                );
        ...
        <tr class="<?php echo esc_attr( $class ); ?>" data-slug="<?php echo esc_attr( $plugin['slug'] ); ?>" data-plugin="<?php echo esc_attr( $plugin['file'] ); ?>" data-status="<?php echo esc_attr( $plugin['status'] ); ?>" data-source="<?php echo esc_attr( $this->get_plugin_source( $plugin ) ); ?>">
            ...
            <th scope="row" class="check-column">
                <label class="screen-reader-text" for="">
                    ...
                    printf( __( 'Select %s', 'stem' ), $plugin['name'] );
                    ?>
                </label>
            ...
            <td class="column-installed-version">
                ...
                <?php else : ?>
                    <?php esc_html_e( 'Not Installed', 'stem' ); ?>
                <?php endif; ?>
            </td>
    ...
    private function display_actions( $plugin ) {
        ...
        if ( $is_wp || 'not-installed' !== $plugin['status'] ) {
            ...
            $actions['details'] = array(
                'url'    => $plugin['url'],
                'text'   => __( 'Details', 'stem' ),
                // translators: 1: name of plugin
                'label'  => sprintf( __( 'More Information about %s', 'stem' ), $plugin['name'] ),
                'target' => '_blank',
                'nonce'  => null,
            );
        ...
        if ( 'not-installed' === $plugin['status'] ) {
            ...
            if ( $is_wp ) {
                ...
                $actions['install'] = array(
                    'url'    => $url,
                    'text'   => __( 'Install', 'stem' ),
                    // translators: 1: name of plugin
                    'label'  => sprintf( __( 'Install %s', 'stem' ), $plugin['name'] ),
                    'target' => '_self',
        ...
        if ( 'inactive' === $plugin['status'] ) {
            ...
            $actions['activate'] = array(
                'url'    => $url,
                'text'   => __( 'Activate', 'stem' ),
                // translators: 1: name of plugin
                'label'  => sprintf( __( 'Activate %s', 'stem' ), $plugin['name'] ),
                'target' => '_self',
            );
            ...
            $actions['deactivate'] = array(
                'url'    => $url,
                'text'   => __( 'Deactivate', 'stem' ),
                // translators: 1: name of plugin
                'label'  => sprintf( __( 'Deactivate %s', 'stem' ), $plugin['name'] ),
                'target' => '_self',
        ...
        if ( 'inactive' === $plugin['status'] ) {
            ...
            $actions['delete'] = array(
                ...
                'text'   => __( 'Delete', 'stem' ),
                // translators: 1: name of plugin
                'label'  => sprintf( __( 'Delete %s', 'stem' ), $plugin['name'] ),
                'target' => '_self',
        ...
        if ( $plugin['update'] ) {
            ...
            $actions['update'] = array(
                ...
                // translators: 1: name of plugin, 2: new version of plugin
                    __( 'Update %1$s to version %2$s', 'stem' ),
                    $plugin['name'],
                    $plugin['new_version']
                ),
                'target' => '_self',
            ...
            $actions['update'] = array(
                'text' => __( 'Plugin is up-to-date', 'stem' ),
            );
    ...
    private function display_status( $plugin ) {
        ...
        switch ( $plugin['status'] ) {
            case 'active':
                esc_html_e( 'Active', 'stem' );
                break;

            case 'incompatible':
                esc_html_e( 'Incompatible', 'stem' );
                break;

            case 'inactive':
                esc_html_e( 'Installed', 'stem' );
                break;

            default:
                esc_html_e( 'Not Installed', 'stem' );
        }
    ...
    public function row_refresh() {
         ...
         * object is used.
         */
        if ( empty( $_POST['namespace'] ) || 'Stem' !== $_POST['namespace'] ) {
            return;
        }
    ...
    public function request() {
        ...
        if ( isset( $_REQUEST['success'] ) ) {
            ...
            if ( 'activate' === $_REQUEST['action'] ) {
                ...
                $message = __( 'Plugin activated.', 'stem' );
                ...
                $message = __( 'Plugin deactivated.', 'stem' );

            } elseif ( 'deactivate-selected' === $_REQUEST['action'] ) {

                $message = __( 'Plugins deactivated.', 'stem' );
        ...
        if ( $do_bulk && empty( $_REQUEST['checked'] ) ) {
            ...
            $error = __( 'No plugins were selected.', 'stem' );

        } elseif ( ! $do_bulk && empty( $_REQUEST['plugin'] ) ) {

            $error = __( 'No plugin slug was given.', 'stem' );

        } elseif ( ! current_user_can( 'update_plugins' ) ) {

            $error = __( 'Sorry, you are not allowed to update plugins for this site.', 'stem' );

        } elseif ( ! $do_bulk && empty( $plugins_data[ $_REQUEST['plugin'] ] ) ) {

            $error = sprintf(
            // translators: 1: slug of plugin being activated
                __( 'The plugin %s doesn\'t exist within the plugin manager\'s registered plugins.', 'stem' ),
                $_REQUEST['plugin']
            );
        ...
        if ( ! $error ) {
            ...
            if ( $do_bulk ) {
                ...
                if ( ! $plugins ) {
                    $error = __( 'No valid plugins given for bulk action.', 'stem' );
                }
            } else {
            ...
            if ( 'activate' === $_REQUEST['action'] ) {
                ...
                if ( $success ) { // At least one plugin needed to be successful.
                    ...
                    $error = __( 'None of the selected plugins could be activated. Make sure they are installed.', 'stem' );
